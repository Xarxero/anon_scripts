...
cd ~/anon
git fetch origin
git checkout v2/final_sapling
git pull origin v2/final_sapling
git clean -dfx
echo \n "To ensure a faster recompiling process your system can spread it as multi-threading "
echo -n "How many CPU-cores does your system support: "
read $cores
./anonutil/build.sh -j $cores
sleep 3s
...
read -p "Press enter to continue: "
sleep 3s
echo \n "Starting Full-Node -> " ./src/anond -reindex
sleep 30s
pkill anond
